...
namespace Logic
{
    public class Boligrafo
    {
        ...
        private const short maxInkAmount = 100;
        private ConsoleColor color;
        private short ink;

        public Boligrafo(ConsoleColor color, short ink)
        {
            this.color = color;
            SetInk(ink);
        }
        ...
        public bool DrawShape(short cost, out string draw)
        {
            draw = "";
            short actualAmountInk = GetInk();
            int wasteInk = Math.Min(cost, actualAmountInk);
